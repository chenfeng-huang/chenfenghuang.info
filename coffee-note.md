...
function measureReferenceBlockHeight(items, perPage) {
  const list = Array.prototype.slice.call(items);
  if (!list.length) return 0;
  const saved = list.map((el) => el.style.display);
  list.forEach((el) => { el.style.display = 'block'; });
  const n = Math.min(perPage, list.length);
  let sum = 0;
  for (let i = 0; i < n; i++) {
    sum += list[i].offsetHeight;
  }
  list.forEach((el, i) => { el.style.display = saved[i] || ''; });
  return sum;
}
...
function initializePagination() {
  ...
  referenceBlockHeight = measureReferenceBlockHeight(postItems, POSTS_PER_PAGE);
...
function applyCoffeePageVisibility(page, options) {
  options = options || {};
  currentPage = page;
  const postItems = document.querySelectorAll('.post-item');
  ...
  if (postsContainer) {
    const itemsOnPage = Math.max(0, Math.min(endIndex, postItems.length) - startIndex);
    if (totalPages > 1 && page === totalPages && itemsOnPage < POSTS_PER_PAGE) {
      postsContainer.style.minHeight = referenceBlockHeight + 'px';
    } else {
...
function relayoutCoffeePagination() {
  const postItems = document.querySelectorAll('.post-item');
  referenceBlockHeight = measureReferenceBlockHeight(postItems, POSTS_PER_PAGE);
  applyCoffeePageVisibility(currentPage, { skipScroll: true });
}

let coffeeArchiveResizeTimer = null;
window.addEventListener('resize', () => {
  if (coffeeArchiveResizeTimer) clearTimeout(coffeeArchiveResizeTimer);
  coffeeArchiveResizeTimer = setTimeout(relayoutCoffeePagination, 120);
});
