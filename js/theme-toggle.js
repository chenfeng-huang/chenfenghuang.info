...
(function() {
    ...
    const LIGHT_MODE_LABEL = 'Switch to light mode';
    const DARK_MODE_LABEL = 'Switch to dark mode';
    ...
    function updateThemeToggle(theme) {
        if (themeToggle) {
            const label = theme === DARK_THEME ? LIGHT_MODE_LABEL : DARK_MODE_LABEL;
    ...
    function watchSystemTheme() {
        if (window.matchMedia) {
            ...
            mediaQuery.addEventListener('change', function(e) {
                // Only auto-switch site theme if user hasn't manually set a preference
                if (!localStorage.getItem(THEME_KEY)) {
