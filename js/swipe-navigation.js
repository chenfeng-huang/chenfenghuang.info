...
(function() {
  ...
  function navigateToPage(direction) {
    ...
    if (nextIndex >= 0 && nextIndex < pages.length) {
      ...
      const transitionDirection = direction > 0 ? 'left' : 'right';

      if (window.pageTransition) {
        window.pageTransition.navigate(nextPage.path, transitionDirection, {
          mobile: window.innerWidth <= 768,
          delayMs: window.innerWidth <= 768 ? 70 : 110
        });
      } else {
  ...
  function init() {
    ...
    pages.forEach(page => {
      if (window.pageTransition) {
        window.pageTransition.prefetch(page.path);
      }
    });
  }
